...
- `#copyMd`, `#goBack` — **마크업 불필요.** doc.js가 만든다
...
> **`#goBack`(뒤로가기)도 마크업이 필요 없다.** `<nav class="doctabs">`가 있는 문서면 doc.js가
> 탭 줄 **왼쪽 여백에 겹쳐서**(absolute) lucide `chevron-left`를 만들어 붙인다 —
> 링크로 다른 문서에 갔다가 돌아오는 자리다. **flow에서 빠지므로 탭 위치는 그대로다.**
> 히스토리가 없으면(새 탭에서 연 첫 문서) 만들지 않고, 1024px 아래에서는 여백이 없어 숨긴다.
> 탭이 없는 문서에는 붙지 않으므로, 문서 사이를 오가게 하려면 그 문서에도 `doctabs`를 둔다. 인쇄 시 숨김.
